...
page 70406 "Mold Picture1"
{
        ...
        area(processing)
        {
            ...
            action(ExportFile)
            {
                ...
                var
                    ...
                    FileName: Text;
                begin
                    if Rec.Picture1.Count = 0 then
                        exit;

                    for Index := 1 to Rec.Picture1.Count do begin
                        if TenantMedia.Get(Rec.Picture1.Item(Index)) then begin
    ...
    begin
        ...
        if UploadIntoStream('Import Picture', '', 'All Files (*.*)|*.*', FromFileName, PicInStream) then begin
            Clear(Rec.Picture1);
            Rec.Picture1.ImportStream(PicInStream, FromFileName);
            Rec.Modify(true);
        end;
    end;
    ...
    begin
        ...
        if Camera.GetPicture(PictureInstream, PictureDescription) then begin
            Clear(Rec.Picture1);
            Rec.Picture1.ImportStream(PictureInstream, PictureDescription, MimeTypeTok);
            Rec.Modify(true);
            exit(true);
        end;
    ...
    begin
        DeleteExportEnabled := Rec.Picture1.Count <> 0;
    end;
